...
class Decoder(nn.Module):
    ...
    def forward(self, x, h_prev_t):
        ...
        #|h_prev_t| = (n_layers, batch_size, hidden_size)

        # Unlike encoder, decoder must take an input for sequentially.
        y, h = self.rnn(x, h_prev_t)
...
class Seq2Seq(nn.Module):
    ...
    def concat_emb_and_hidden(self, emb_t, h_t_tilde):
        if h_t_tilde is None:
            batch_size = emb_t.size(0)
            # If this is the first time-step,
            h_t_tilde = emb_t.new(batch_size, 1, self.hidden_size).zero_()

        return torch.cat([emb_t, h_t_tilde], dim =-1)
    ...
    def forward(self, src, tgt):
        ...
        for t in range(tgt.size(1)):
            ...
            emb_current_t = emb_tgt[:, t, :].unsqueeze(1)
            # |emb_current_t| = (batch_size, 1, word_vec_size)
            # |h_t_tilde| = (batch_size, 1, hidden_size)

            x = self.concat_emb_and_hidden(emb_current_t, h_t_tilde)

            decoder_output, decoder_hidden = self.decoder(x, decoder_hidden)
    ...
    def batch_beam_search(
        ...
        while sum(is_done) < batch_size and length <= max_length:
            ...
            x = self.concat_emb_and_hidden(emb_t, fab_h_t_tilde)
            fab_decoder_output, (fab_hidden, fab_cell) = self.decoder(x, (fab_hidden, fab_cell))
