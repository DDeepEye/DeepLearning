...
class Transformer(nn.Module):
    ...
    def _generate_pos_enc(self, hidden_size, max_length):
        ...
        pos = pos / (1e+4**dim.div(float(hidden_size)))
    ...
    def search(self, x, is_greedy=True, max_length=255):
        ...
        emb_output = self.emb_dropout(self._position_encoding(self.emb_enc(x)))
        encoder_output, _ = self.encoder(emb_output, mask_enc)
        # |z| = (batch_size, n, hidden_size)
        ...
        prevs = [ConcatTensor(concat_dim=1) for _ in range(len(self.decoder._modules) + 1)]
        y_hats, indice = [], []
        ...
        while is_decoding.sum() > 0 and len(indice) < max_length:
            ...
            for layer_index, block in enumerate(self.decoder._modules.values()):
                prev = prevs[layer_index].tensor
                # |prev| = (batch_size, len(y_hats), hidden_size)

                h_t, _, _, _, _ = block(h_t, encoder_output, mask_dec, prev, None)
                # |h_t| = (batch_size, 1, hidden_size)
    ...
    def batch_beam_search(
        ...
        while sum(done_cnt) < batch_size and length <= max_length:
            fab_input, fab_z, fab_mask = ConcatTensor(0), ConcatTensor(0), ConcatTensor(0)
            fab_prevs = [ConcatTensor(0) for _ in range(n_dec_layers + 1)]
            ...
            for i, board in enumerate(boards): # i == sample_index in minibatch
                if board.is_done() == 0:
                    ...
                    for layer_index in range(n_dec_layers + 1):
                        fab_prevs[layer_index] += prev_status['prev_state_%d' % layer_index]
            ...
            # take the last time-step's output during the inference.
            h_t = self.emb_dropout(
                self._position_encoding(self.emb_dec(fab_input.tensor), init_pos=length)
            )
            # |h_t| = (current_batch_size, 1, hidden_size)
            for i in range(len(fab_prevs)):
                fab_prevs[i].concat_dim = 1

            fab_prevs[0] += h_t

            for layer_index, block in enumerate(self.decoder._modules.values()):
                prev = fab_prevs[layer_index].tensor
                # |prev| = (current_batch_size, m, hidden_size)

                h_t, _, _, _, _ = block(h_t, fab_z.tensor, fab_mask.tensor, prev, None)
                # |h_t| = (current_batch_size, 1, hidden_size)
