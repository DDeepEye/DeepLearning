...
class SingleBeamSearchBoard():
    ...
    def collect_result(self, y_hat, prev_status):
        ...
        #     |prev_state_i| = (beam_size, length, hidden_size),
        #     where i is an index of layer.
        output_size = y_hat.size(-1) # y_hat은 문자 분절들 확률의 ln를 취한 값
        ...
        if prev_status is not None :
            for status_name, status in prev_status.items():
                self.prev_status[status_name] = torch.index_select(
                    status,
                    dim=self.batch_dims[status_name],
