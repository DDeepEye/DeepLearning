...
if __name__ == '__main__':    
    ...
    tr_args = NMTTranslateArg(
        model_filepath=os.path.dirname(__file__)+'/2021.1004.TFM/Transformer.pth'
        ,translate_filepath=os.path.dirname(__file__)+'/corpus/10_sampling.test.en.tr'
        ,output_filepath=os.path.dirname(__file__)+'/corpus/translate.kr.txt'
        ,gpu_id=0
        ,batch_size = 5
        ,beam_size = 10
        ,length_penalty = 5
            ...
            while is_trans:
                ...
                if config.beam_size == 1:
                    ...
                    for i in range(len(output)):
                        output[i] = output[i].replace(' ','').replace(STR+STR, ' ').replace(STR, '').replace(TRANS_STR,' ')
                        output[i] += '\n'
                    ...
                    for i in range(len(output)):
                        for j in range(len(output[i])):
                            output[i][j] = output[i][j].replace(' ','').replace(STR+STR, ' ').replace(STR, '').replace(TRANS_STR,' ')
                            output[i][j] += '\n'
